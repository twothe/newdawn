...
public class ChunkProviderGenerate implements IChunkProvider {
  ...
  protected final NoiseStretch heightChunkNoise;
...
//  private final TimeCounter timeDecorate = new TimeCounter("Decorate");
  ...
  public ChunkProviderGenerate(World par1World, long worldSeed, boolean useMapFeatures) {
    this.seedRandom = getRandomGenerator(worldSeed);
    worldNoise = new SimplexNoise(seedRandom);
    this.heightChunkNoise = new NoiseStretch(worldNoise, 33.0, 37.0);
  ...
  public Chunk provideChunk(int chunkX, int chunkZ) {
    byte chunkData[] = new byte[16 * 16 * 256];
...
//    timeChunk.start();
    ...
    BiomeGenBase.extremeHills.topBlock = stoneID;
    BiomeGenBase.extremeHills.fillerBlock = stoneID;
    BiomeGenBase.ocean.fillerBlock = sandID;
    BiomeGenBase.ocean.topBlock = sandID;

    final double x0 = chunkX << 4;
    final double z0 = chunkZ << 4;
    final int seaLevel = (int) worldObj.getSeaLevel();
    ...
    for (x = 0; x < 16; ++x) {
      for (z = 0; z < 16; ++z) {
        ...
        final double varianceChunk = (this.worldNoise.noise(blockX / 324.0, blockZ / 298.0) + 1.0) * 1.0;
        final double varianceArea = (this.worldNoise.noise(blockX / 344.0, blockZ / 378.0) + 1.0) * 8.0;
        final double varianceRegion = (this.worldNoise.noise(blockX / 1000.0, blockZ / 978.0) + 1.0) * 16.0;
        ...
        if (hillsNoiseEffective >= 0.0) {
          ...
          if (hillsNoiseEffective >= 0.01) {
            ...
            hillsHeight = hillsNoiseEffective * 52.0;
          }
        } else {
        ...
        final int elevationArea = elevationRegion + (int) Math.round(areaHeight + hillsHeight);
        final int elevationChunk = elevationArea + (int) Math.round(chunkHeight);
        final int height = Math.min(255, seaLevel + 1 + (int) Math.round(regionHeight + areaHeight + hillsHeight + chunkHeight));
